...
The raw upstream driver (``vendor/lmc/linear_motor_controller.py``) only accepts a
device path (``/dev/ttyUSBn``), which renumbers across reboots/re-plugs. On
this bench the rail is wired through a Moxa UPort 1150 (``110A:1150``), so this
thin shim adds VID:PID resolution — the one driver whose upstream lacks it
(sy01b/entris_ii resolve internally). Pass a device path or a ``"VID:PID"``
string; it is resolved at open time.
"""
...
class LinearMotorController(_LinearMotorController):
    """MINAS A6 driver that also accepts a ``"VID:PID"`` port string."""
